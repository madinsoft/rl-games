...
# viz = EsVizu('wins', 'note', 'reward', dt=2)
measurement = 'deepQ'
viz = SdiVizu('wins', 'note', 'reward', dt=4, measurement=measurement, model_name=f'hid{num_hidden}')
    ...
    for timestep in range(1, max_steps_per_episode):
        ...
        if frame_count % update_after_actions == 0 and len(done_history) > batch_size:
            ...
            if frame_count % 100 == 0:
                ...
                note = evaluate2(model)
                viz(wins, note, episode_reward)
                print(f'episode {episode_count}, frame_count {frame_count}, timestep {timestep}, wins {wins}, note {note}, {episode_reward}')
                if wins == 100:
                    model.save(f'/home/patrick/projects/IA/my-2048/data_models/deep2_{num_hidden}')
                    viz.freeze()
                    exit()
